...
void setup() {
  ...
  Thread leapObserver = new Thread(new HandPublisher(leap));
  leapObserver.run();
}
...
public class HandPublisher extends java.util.Observable implements Runnable {
  ...
  public void run() {
    while(true) {
      boolean test = checkHandSpread();
    }
  }
  
  private boolean checkHandSpread() {
    ArrayList<PVector> fingerPositions = new ArrayList<PVector>(); 
    if(lm.getHands().isEmpty())
      return false;
    for (Finger f : this.lm.getHands().get(0).getFingers()) {
      fingerPositions.add(f.getPosition());
    }
    return getMaxDistance(fingerPositions) > 30;
  }
  
  private float getMaxDistance(List<PVector> positions) {
    float maxDist = 0;
    float currentDist = 0; //<>//
    System.out.println(positions.toString());
    // Starting with one to skip thump
    for(int i = 1; i < positions.size()-1; i++) {
       currentDist = positions.get(i).dist(positions.get(i+1));
       System.out.printf(currentDist + " ");
  ...
  for (Hand hand : leap.getHands ()) {
    ...
    for (Finger finger : hand.getFingers()) {
      ...
      case 1:
        ...
        System.out.println(finger.getPosition().toString());
        break;
      case 2:
